...
<body class="bg-gray-400 min-h-screen flex items-center justify-center p-4">
    <div class="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 sm:p-10">
        <div class="text-center mb-8 sm:mb-10">
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">Form Login</h1>
            {{-- Alert sukses --}}
            @if (session('success'))
            <div 
                id="successToast"
                class="fixed top-4 left-1/2 transform -translate-x-1/2 bg-green-500 text-black text-base px-6 py-3 rounded-xl shadow-lg transition-all duration-300">
                {{ session('success') }}
            </div>
            <script>
            ...
            {{-- Alert error --}}
            @if ($errors->any())
            <div class="bg-red-500 text-white text-base px-6 py-3 rounded-lg shadow-lg mb-4">
